...
export class WebhookService {
  ...
  private createMessagePayload(request: WebhookRequest): MessagePayload {
    ...
    if (request.embed) {
      ...
      if (request.embed.description) {
        request.embed.description = ReplaceGitHubMentions(request.embed.description)[0];
      }
      ...
      if (request.embed.fields && Array.isArray(request.embed.fields)) {
        request.embed.fields = request.embed.fields.map(field => ({
          ...field,
          value: field.value ? ReplaceGitHubMentions(field.value)[0] : field.value
        }));
      }
  ...
  private validateRequest(targetId: string, request: WebhookRequest, res: express.Response): boolean {
  ...
  private handleChannelMessage = async (req: express.Request, res: express.Response): Promise<express.Response> => {
    try {
  ...
  private handleUserMessage = async (req: express.Request, res: express.Response): Promise<express.Response> => {
    try {
