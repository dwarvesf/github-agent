import { Interaction, ModalSubmitInteraction, GuildMember } from 'discord.js'
...
export class ModalHandler implements EventHandler {
    ...
    try {
      ...
      if (!interaction.replied && !interaction.deferred) {
        await InteractionUtils.send(interaction, {
          content:
            'Something went wrong when processing your submission. Please try again.',
          ephemeral: true,
        })
      }
    }
  ...
  private async handleAccessKeyModal(
    ...
    try {
      ...
      const submittedKey =
        interaction.fields.getTextInputValue('access-key-input')
      ...
      if (submittedKey === validKey) {
        ...
        const privateRole = interaction.guild?.roles.cache.find(
          (role) => role.name === 'PrivateAccess',
        )
        ...
        if (privateRole) {
          ...
          await interaction.reply({
            content:
              '✅ **Access Granted!** You now have access to the private channels.',
            ephemeral: true,
          })
        } else {
          await interaction.reply({
            content:
              '❌ Configuration error: Private role not found. Please contact an administrator.',
            ephemeral: true,
          })
        }
      } else {
        ...
        await interaction.reply({
          content:
            '❌ **Invalid key!** The access key you provided is not correct. Please try again or contact an administrator.',
          ephemeral: true,
        })
      }
    } catch (error) {
      ...
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({
          content:
            '❌ An error occurred while processing your access key. Please try again.',
          ephemeral: true,
        })
      }
    }
